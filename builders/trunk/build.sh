#!/bin/sh
# Build script for trunk version of RNO-G software
...
RNO_G_BUILD_DIR="${DEST%/}/RNO_G_build/"
DEPS_BUILD_DIR="${DEST%/}/misc_build/"
ROOT_BUILD_DIR="${DEST%/}/root_build/"
if [ ! -d "$SOURCE_DIR" ]; then
	mkdir "$SOURCE_DIR"
fi
if [ ! -d "$RNO_G_BUILD_DIR" ]; then
	mkdir "$RNO_G_BUILD_DIR"
fi
...
./build_mattak.sh --source "$SOURCE_DIR" --build "$RNO_G_BUILD_DIR" --root "$ROOT_BUILD_DIR" --deps "$DEPS_BUILD_DIR" $MAKE_ARG $SKIP_ARG || error 108 "Failed libRootFftwWrapper build"
...
echo "Finished building RNO-G software"